...
int mb_load_controller_config(){
    ...
    if(rc_filter_pid(&pid_filt_l, 
        ...
        l_wheel_speed_params.ki,
        l_wheel_speed_params.kd,
        l_wheel_speed_params.dFilterHz*4,
    ...
    rc_filter_pid(&pid_filt_r, 
        ...
        r_wheel_speed_params.ki,
        r_wheel_speed_params.kd,
        r_wheel_speed_params.dFilterHz*4,
        r_wheel_speed_params.dFilterHz);
...
int mb_controller_update(mb_state_t* mb_state, mb_setpoints_t* mb_setpoints){
    mb_state->left_cmd = rc_filter_march(&pid_filt_l, mb_setpoints->fwd_velocity + ((float) mb_state->left_encoder_delta)/(1.0/SAMPLE_RATE_HZ));
    mb_state->right_cmd = rc_filter_march(&pid_filt_r, mb_setpoints->fwd_velocity + ((float) mb_state->right_encoder_delta)/(1.0/SAMPLE_RATE_HZ));
