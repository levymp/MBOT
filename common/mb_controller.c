...
int mb_controller_update(mb_state_t* mb_state, mb_setpoints_t* mb_setpoints){
    mb_state->left_cmd = rc_filter_march(&pid_filt_l, mb_state->fwd_velocity - ((float) mb_state->left_encoder_delta)/(1.0/SAMPLE_RATE_HZ));
    mb_state->right_cmd = rc_filter_march(&pid_filt_r, mb_state->fwd_velocity - ((float) mb_state->right_encoder_delta)/(1.0/SAMPLE_RATE_HZ));
    return 0;
}
